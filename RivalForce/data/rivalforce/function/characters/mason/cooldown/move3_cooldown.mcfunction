...
# If Still in Invincibility
execute at @p[scores={charID=2, moveSlotExtra3=1..}] run item replace entity @p[scores={charID=2}] hotbar.3 with diamond[custom_name={"bold":true,"color":"#8F8F8F","italic":true,"text":"Stone Armor"},lore=[{"color":"#5e5e5e","italic":true,"text":"Protects Mason from attacks, but locks him in place."}],item_model="rival_force_resources:mason_skills/mason_stone_armor_cooldown"] 1
execute at @p[scores={charID=2, moveSlotExtra3=1..}] run scoreboard players set @p[scores={charID=2}] moveSlotCooldown3 450
execute at @p[scores={charID=2, moveSlotExtra3=1..}] run playsound minecraft:entity.generic.explode master @a[distance=..18] ~ ~ ~ 100 1.0 0
execute at @p[scores={charID=2, moveSlotExtra3=1..}] run particle block{block_state:"minecraft:stone"} ^ ^ ^ 0.4 1 0.4 2 800 normal
execute at @p[scores={charID=2, moveSlotExtra3=1..}] run particle explosion ^ ^1 ^ 0.1 0.1 0.1 1 3 normal

# Exit Damage
execute at @p[scores={charID=2, moveSlotExtra3=1..}] as @e[distance=..2.5] run execute unless score @p Affiliation = @s Affiliation unless entity @s[tag=invincible] unless entity @s[tag=stage] run damage @s 6 minecraft:player_attack

# Otherwise...
execute at @p[scores={charID=2, moveSlotExtra3=0}] run item replace entity @p hotbar.3 with warped_fungus_on_a_stick[custom_name={"bold":true,"color":"#8F8F8F","italic":true,"text":"Stone Armor"},lore=[{"color":"#5e5e5e","italic":true,"text":"Protects Mason from attacks, but locks him in place."}],custom_data={moveSlot:3},item_model="rival_force_resources:mason_skills/mason_stone_armor"] 1
execute at @p[scores={charID=2, moveSlotExtra3=0}] run scoreboard players reset @p moveSlotCooldown3

# Restore Moves
execute at @p[scores={charID=2, moveSlotExtra3=0, moveSlotCooldown0=..0}] run item replace entity @p hotbar.0 with warped_fungus_on_a_stick[custom_name={"bold":true,"color":"#8F8F8F","italic":true,"text":"Boulder Shot"},lore=[{"color":"#5e5e5e","italic":true,"text":"A slow-moving boulder projectile shot straight ahead."}],custom_data={moveSlot:0},item_model="rival_force_resources:mason_skills/mason_boulder_shot"] 1

execute at @p[scores={charID=2, moveSlotExtra3=0, moveSlotCooldown2=..0}] run item replace entity @p hotbar.2 with warped_fungus_on_a_stick[custom_name={"bold":true,"color":"#8F8F8F","italic":true,"text":"Stone Pillar"},lore=[{"color":"#5e5e5e","italic":true,"text":"Creates a stone pillar beneath Mason."}],custom_data={moveSlot:2},item_model="rival_force_resources:mason_skills/mason_stone_pillar"] 1

tag @p[scores={charID=2}] remove invincible
