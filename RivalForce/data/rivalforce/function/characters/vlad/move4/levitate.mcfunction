...
execute at @p[scores={charID=1, itemActivation=1..}] run scoreboard players add @p moveSlotExtra4 1
execute at @p[scores={charID=1, moveSlotExtra4=2.., itemActivation=1..}] run scoreboard players set @p moveSlotExtra4 0
...
# Cooldown
item replace entity @p[scores={charID=1, itemActivation=1..,moveSlotExtra4=1}] hotbar.4 with warped_fungus_on_a_stick[custom_name={"bold":true,"color":"#d50064","italic":true,"text":"Cancel Levitate"},lore=[{"color":"#740036","italic":true,"text":"Cancels Vlad's Levitate."}],custom_data={moveSlot:4},item_model="rival_force_resources:vlad_skills/vlad_levitate_exit"] 1
